...
function renderChecklist() {
  ...
  checklistData.forEach((section) => {
    ...
    suggestBtn.addEventListener('click', () => {
      ...
      if (suggestion) {
        insightsEl.innerHTML = `<strong>AI Suggestion</strong>${suggestion}`;
        insightsEl.classList.add('is-visible');
        insightsEl.classList.remove('is-success');
        insightsEl.dataset.tone = 'info';
      } else {
        insightsEl.innerHTML = '<strong>AI Suggestion</strong>No suggestion available.';
        insightsEl.classList.add('is-visible');
        insightsEl.classList.remove('is-success');
        insightsEl.dataset.tone = 'info';
      }
    ...
    crossCheckBtn.addEventListener('click', () => {
      ...
      if (section.crossCheck) {
        ...
        insightsEl.innerHTML = `<strong>Cross-check</strong>${message}`;
        insightsEl.classList.add('is-visible');
...
function init() {
  ...
  previewBody = previewOverlay ? previewOverlay.querySelector('.preview__body') : null;
  if (previewOverlay) {
